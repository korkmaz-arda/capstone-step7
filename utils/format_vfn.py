...
root_dir = ""
base_dir = f'{root_dir}/datasets/vfn-yolo'
vfn_dir = f'{root_dir}/datasets/vfn'
...
train_ids = load_image_ids(os.path.join(meta_dir, 'training.txt'))
val_ids = load_image_ids(os.path.join(meta_dir, 'validation.txt'))
test_ids = load_image_ids(os.path.join(meta_dir, 'testing.txt'))

print("Image IDs loaded for train, val, and test splits.")
...
def process_annotations(annotation_file, split_ids, split):
    ...
    with open(annotation_file, 'r') as f:
        for line_count, line in enumerate(f, 1):
            ...
            if line_count % 100 == 0:
                print(f"Processed {line_count} annotations for {split} set.")

    print(f"Completed processing annotations for {split} set.")


process_annotations(os.path.join(meta_dir, 'annotations.txt'), train_ids, 'train')
process_annotations(os.path.join(meta_dir, 'annotations.txt'), val_ids, 'val')
process_annotations(os.path.join(meta_dir, 'annotations.txt'), test_ids, 'test')

print("Dataset conversion to YOLO format completed.")
